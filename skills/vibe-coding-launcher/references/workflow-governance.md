...
本文件总结 AI 协作开发中最值得复用的项目规范。它回答三个问题：
...
**非平凡任务**是指需要先形成可审查的 spec 和 plan，再进入实现的任务。只要满足以下**任一**条件，即为非平凡任务：

### 明确的判断标准

| 维度 | 判断标准 | 示例 |
|------|---------|------|
| **用户可见影响** | 改变用户可见行为或用户体验 | 新增功能、修改UI界面、改变API响应格式 |
| **边界新增** | 新增产品、架构、数据、部署或安全边界 | 新增模块、新增数据表、新增外部服务依赖 |
| **风险等级** | 影响认证、权限、持久化、数据安全或部署 | 修改认证逻辑、改变数据持久化方式、修改权限控制 |
| **范围大小** | 跨多个模块或目录修改 | 同时修改 `ui/`、`service/`、`repo/` 三层 |
| **时间预估** | 预估需要超过 30 分钟完成 | 需要数小时到数天的功能开发 |
| **架构变更** | 新增层级、新依赖方向、新抽象层 | 引入新的设计模式、重构核心架构 |
| **多人协作** | 需要与他人协作或 review | 涉及多人代码合并、需要团队确认 |

### 轻量路径的条件

**只有同时满足以下所有条件**，才可跳过完整 spec → plan → tasks 流程，使用轻量路径：

- ✅ 低风险：不影响认证、权限、数据安全或核心功能
- ✅ 改动范围小：只修改单个模块或目录，不涉及共享接口/类型
- ✅ 无新边界：不新增产品、架构、数据、部署或安全边界
- ✅ 无实质变更：不改变认证、权限、持久化、API contract 或运行时行为
- ✅ 时间可控：预估 30 分钟内可以完成
- ✅ 单模块：只改单个目录且不触及共享接口/类型
